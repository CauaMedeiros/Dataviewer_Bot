...
class Bot:
    def __init__(self, api_key, profile_number=0):
        # Carrega a chave da API do ambiente e inicializa os componentes do bot
        ...
        # Defin as variáveis para o banco de dados vetorial e histórico de conversas
        self.persist_directory = "./db"
        self.max_history = 5
        self.conversation_history = []
        self.history_handler = history_handler()

        # Configura o prompt do chatbot com base no perfil selecionado
        ...
        # Configura o mecanismo de recuperação de contexto
        self.retriever = self.vectordb.as_retriever(
            search_type="similarity_score_threshold",
            search_kwargs={"score_threshold": 0.2}
        )

        # Combina os documentos recuperados com o prompt para gerar a resposta do chatbot
    ...
    # Cria o banco de dados vetorial a partir dos documentos, dividindo-os em chunks e gerando embeddings
    def create_vector_db(self, files_dir, chunk_size=1000, chunk_overlap=200):
        
        # Lê o documento e divide em chunks
        loader = TextLoader(file_path=files_dir, encoding="utf-8")
        documents = loader.load()
        
        # Divide o texto em chunks menores para melhor processamento e geração de embeddings
        ...
        # Cria o banco de dados vetorial usando os chunks e os embeddings gerados
    ...
    # Formata o histórico de conversas para ser incluído no prompt, mantendo apenas as últimas interações relevantes
    ...
    # Gera a resposta do chatbot com base na pergunta do usuário, no histórico de conversas e no contexto recuperado dos documentos
    def generate_response(self, query: str, user_id: str) -> Dict:
        # Prepara o input baseado na pergunta do usuário e no histórico de conversas formatado
        input_dict = {
            "input": query,
            "conversation_history": self._format_conversation_history(user_id)
        }
        
        # Chama a cadeia de recuperação para obter o contexto relevante e gerar a resposta do chatbot
        result = self.retrieval_chain.invoke(input_dict)
        
        # Atualiza o histórico de conversas com a nova interação, mantendo apenas as últimas interações relevantes para evitar sobrecarregar o contexto
        if "answer" in result:
            self.conversation_history.append((query, result["answer"]))

            # Mantem apenas as últimas interações relevantes no histórico para evitar sobrecarregar o contexto
...
# Classe para gerenciar o histórico de conversas dos usuários, armazenando e recuperando as interações
